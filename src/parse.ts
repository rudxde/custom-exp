...
function tryParseOperation(left: Expression, index: number, tokens: Tokens): canFail<Operation> {
    ...
    const rightResult = tryParseExpression(index, tokens);
...
function tryParseOperator(index: number, tokens: Tokens): canFail<Operator> {
    ...
    const dualCharOperators = ['&&', '||'];
    if (singleCharOperators.includes(nextToken)) return { increasedIndex: index, result: <Operator>nextToken };
    if (dualCharOperators.includes(nextToken + overNextToken)) return { increasedIndex: index + 1, result: <Operator>(nextToken + overNextToken) };
    return FAIL;
}
...
export function tryParseExpression(index: number, tokens: Tokens): canFail<Expression> {
    ...
    while (true) {
        ...
        if (functionExpressionResult === FAIL) break;
    ...
    while (true) {
        ...
        if (operationExpressionResult === FAIL) break;
